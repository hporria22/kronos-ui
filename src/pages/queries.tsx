...
import { queryTypes, sampleSql, queryResults, type QueryResult } from "../data/mockData";
...
const paramsByType: Record<string, ParamDef[]> = {
  ep_attendance: [
    { name: "plant", label: "Plant Code", placeholder: "P001" },
    { name: "month", label: "Month", placeholder: "2026-02" },
  ],
  wo_active: [{ name: "bt", label: "Business Type", placeholder: "APL" }],
  kpi_failed: [{ name: "kpi_type", label: "KPI Type", placeholder: "Data Quality" }],
  plant_compliance: [{ name: "bt", label: "Business Type", placeholder: "APL" }],
  intra_transfers: [],
};
...
const Queries: React.FC = () => {
  const [type, setType] = useState<string>("ep_attendance");
  const [advanced, setAdvanced] = useState<boolean>(false);
  const [sql, setSql] = useState<string>(sampleSql["ep_attendance"]);
  const [params, setParams] = useState<Record<string, string>>({});
  const [result, setResult] = useState<QueryResult | null>(null);
  const [running, setRunning] = useState<boolean>(false);

  useEffect(() => {
    setSql(sampleSql[type] || "");
    setParams({});
  }, [type]);

  const dynamicParams = paramsByType[type] || [];
  ...
  const runQuery = (): void => {
    ...
    setTimeout(() => {
      ...
      setRunning(false);
      toast.success(`Returned ${queryResults[type].rows.length} rows in 142 ms`);
    }, 650);
  };
    ...
    <div className="space-y-6" data-testid="queries-page">
      <div>
        ...
        <p className="text-[13.5px] text-slate-500 mt-1">
          Run pre-defined system queries or compose your own against the Kronos warehouse.
        </p>
      </div>
      ...
      <section className="k-card space-y-5" data-testid="query-card">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="md:col-span-1 space-y-1.5">
            ...
            <Select value={type} onValueChange={setType}>
              <SelectTrigger data-testid="query-type-select" className="h-10 bg-[#F8FAFC]">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {queryTypes.map((q) => (
          ...
          {dynamicParams.map((p) => (
            <div key={p.name} className="space-y-1.5">
              <Label className="k-label">{p.label}</Label>
              <Input
        ...
        <label className="inline-flex items-center gap-2.5 cursor-pointer select-none" data-testid="advanced-toggle">
          ...
          <span className="text-[13.5px] font-medium text-slate-900 flex items-center gap-1.5">
            <Sparkles className="w-3.5 h-3.5 text-[color:var(--teal)]" /> System Query (edit raw SQL)
          </span>
        </label>
